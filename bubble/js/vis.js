...
var simulation = d3.forceSimulation()
        .force("forceX", d3.forceX().strength(.1).x(200 * .5))
        .force("forceY", d3.forceY().strength(.1).y(200 * .5))
        .force("center", d3.forceCenter().x(100 * .5).y(100 * .5))
...
function makeOrdinalVis(error, data){
    ...
    function updateVisualization(){
        ...
        simulation
            .force("forceX", d3.forceX().strength(.1).x(max_height * .5))
            .force("forceY", d3.forceY().strength(.1).y(max_height * .5))
            .force("center", d3.forceCenter().x(max_height * .5).y(max_height * .5))

        vis_svg.transition().duration(0).style("opacity", 0)
            .on("end", function(){
                vis_svg.remove();
                vis_svg = svg.append("g").style("opacity", 1);
                ...
                for(var i in visualization_data.d1){
                    ...
                    if(question2 !== ""){
                        new createBubble(g, visualization_data.d2[i].children)
    ...
    function createBubble(g, vis_data){
        d3.forceSimulation()
            .force("forceX", d3.forceX().strength(.1).x(max_height * .5))
            .force("forceY", d3.forceY().strength(.1).y(max_height * .5))
            .force("center", d3.forceCenter().x(0).y(0))
            .nodes(vis_data)
            .force("collide", d3.forceCollide().strength(.5).radius(function(d){ return d["count"]/10 + 1; }).iterations(1))
            .on("tick", function(d){
                node
